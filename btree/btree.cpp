...
int main (int argc, char* argv[]){
    ...
    std::vector<std::pair<KEY_TYPE, PAYLOAD_TYPE>> *vect = new std::vector<std::pair<KEY_TYPE, PAYLOAD_TYPE>>;
    ...
    // Generating num_keys amount of pairs and insert them into values
        ...
        for (int i = 0; i < num_keys; i++) {
	        ...
	        (*vect).push_back(std::make_pair(key, payload));
        }
    #endif
    #ifdef CONSECUTIVE
        for (int i = 0; i < num_keys; i++) {
	        (*vect).push_back(std::make_pair(static_cast<KEY_TYPE>(i), payload_dis(gen)));
        }
    #endif
    ...
    // Insert all values
    index.bulk_load((*vect).begin(), (*vect).end());
    //Stopwatch
    #ifdef TIME
    ...
    for (int i = 0; i < num_requests ; i++){
        ...
        if (r < percent_read){
            ...
            //Perform deletion
            KEY_TYPE randKey = key_dis(gen);
            auto exists = index.exists(randKey);
            if (exists == true){         
